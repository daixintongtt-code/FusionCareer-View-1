...
前端页面和 Excel 模板已按学院第一批、第二批大/小实习岗位表调整。后端接入时如需调整路径，只需修改 `src/lib/jobImport.mjs` 中的 `JOB_IMPORT_ENDPOINT`。
...
优先解析新模板中名为 `岗位数据` 的工作表。第 1 行为固定表头，第 2 行起每行代表一个岗位；空白行忽略。`填写示例`、`填写说明`和 `旧表映射` 不参与导入。
...
新旧表头和系统字段的完整映射记录在 `public/templates/job-import-template.xlsx` 的 `填写说明` 和 `旧表映射` 工作表。
...
- `薪资/补贴说明（不对外）` 默认不写入 `salaryMin`、`salaryMax`或 `salaryDisplay`。
